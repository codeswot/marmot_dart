...
class _GroupCardState extends State<GroupCard> {
  ...
  void initState() {
    super.initState();
    vm = GroupViewModel(widget.device, widget.group)..addListener(() => setState(() {}));
  }

  @override
  void dispose() { vm.dispose(); _msgCtrl.dispose(); super.dispose(); }

  Widget _renderMsg(BuildContext c, ChatMessage msg) {
    if (!msg.isFile) {
      return Padding(
        padding: const EdgeInsets.symmetric(vertical: 2),
        child: Text('[${msg.sender}] ${msg.text ?? ''}', style: Theme.of(c).textTheme.bodySmall),
      );
    }
    ...
    return Card(
      ...
      child: ListTile(
        ...
        title: Text('[${msg.sender}] ${ref.filename}', style: Theme.of(c).textTheme.bodySmall),
        subtitle: Text(
  ...
  Widget _avatar() {
    if (vm.imageBytes != null) {
      return CircleAvatar(radius: 18, backgroundImage: MemoryImage(vm.imageBytes!));
    }
    return CircleAvatar(
      radius: 18,
        ...
        content: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                TextButton.icon(
                  onPressed: vm.busy ? null : () { Navigator.pop(dctx); vm.doSetGroupImage(); },
                  icon: const Icon(Icons.image, size: 16),
                  label: const Text('Set image'),
                ),
                if (vm.imageBytes != null)
                  TextButton.icon(
                    onPressed: vm.busy ? null : () { Navigator.pop(dctx); vm.clearGroupImage(); },
  ...
  Widget build(BuildContext c) => Card(
    child: ExpansionTile(
      leading: _avatar(),
      title: Row(children: [
        Expanded(child: Text('${vm.name} (${vm.memberCount}m)${vm.messages.isNotEmpty ? " • ${vm.messages.length}" : ""}')),
        if (vm.busy)
          const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2))
        else
          IconButton(icon: const Icon(Icons.edit, size: 18), visualDensity: VisualDensity.compact, onPressed: () => _showEditDialog(c)),
        IconButton(icon: const Icon(Icons.refresh, size: 18), visualDensity: VisualDensity.compact, onPressed: vm.refresh),
      ]),
      subtitle: Text(vm.description.isNotEmpty ? vm.description : vm.id.substring(0, 12)),
      ...
      children: [
        Padding(
          ...
          child: TextField(
            ...
            onSubmitted: (t) { vm.doSendMessage(t); _msgCtrl.clear(); },
          ),
        ),
        Row(children: [
          TextButton.icon(onPressed: () { vm.doSendMessage(_msgCtrl.text); _msgCtrl.clear(); }, icon: const Icon(Icons.send, size: 16), label: const Text('Send')),
          TextButton.icon(onPressed: () { vm.doSendFile(_msgCtrl.text); _msgCtrl.clear(); }, icon: const Icon(Icons.attach_file, size: 16), label: const Text('File')),
        ]),
